...
from common.schema import (
    ClassModel, CourseModel, StudentModel, TeacherModel, UserModel)
...
engine = create_engine(f'mysql+pymysql://'
                       ...
                       f'{dbSettings.get("database")}', poolclass=NullPool)
...
class StudentORM(Base):
    ...
    def name_or_phone_duplicated(cls, name=None, phone=None):
        if name or phone:
            if name:
                instance = db_session.query(cls).filter(
                    cls.name == name).first()
                if instance:
                    raise FastAPIError(1006)
            if phone:
                instance = db_session.query(cls).filter(
                    cls.phone == phone).first()
                if instance:
                    raise FastAPIError(1006)
        else:
...
class UserORM(Base):
    ...
    def user_login(cls, user: UserModel):
        ...
        if instance:
            if bcrypt.checkpw(user.password.encode(),
                              instance.password.encode()):
                return instance.id
        return False
    ...
    def user_name_duplicated_for_update(cls, name, user_id):
        instance = db_session.query(cls).filter(
            cls.name == name, cls.id != user_id).first()
        if instance:
            raise FastAPIError(1006)
